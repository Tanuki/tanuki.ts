...
const LLM_GENERATION_PARAMETERS = ["temperature", "top_p", "max_new_tokens", "frequency_penalty", "presence_penalty"];
...
export class TogetherAIAPI implements LLMApi, LLMFinetuneAPI {
    ...
    public async generate(model: TogetherAIConfig,
        ...
        while (retryCount <= maxRetries) {
            try {
                ...
                if (retryCount === maxRetries) {
                    throw new Error(`Together AI API failed to generate a response: ${error.message}`);
                }
                retryCount++;
    ...
    private formatPrompt(model: TogetherAIConfig, systemMessage: string, userPrompt: string): string {
        let finalPrompt = '';
        const chatPrompt = model.chatTemplate;

        if (chatPrompt) {
            finalPrompt = chatPrompt.replace('{system_message}', systemMessage).replace('{user_prompt}', userPrompt);
        } else {
            try {
                const promptFormat = this.modelConfigs[model.modelName].config.prompt_format as string;
                finalPrompt = promptFormat.replace('{system_message}', systemMessage).replace('{prompt}', userPrompt);
            } catch (error) {
                console.warn("Chat prompt is not defined for this model. Please define it in the model config. Using default chat prompt");
                finalPrompt = `[INST]{system_message}[/INST]\n{user_prompt}`.replace('{system_message}', systemMessage).replace('{user_prompt}', userPrompt);
            }
        }
