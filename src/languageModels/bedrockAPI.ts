...
export class BedrockAPI {
  ...
  constructor() {
    this.bedrockRuntime = new BedrockRuntimeClient({ region: process.env.AWS_REGION || 'us-east-1' });
  }
  ...
  async sendApiRequest(model: BaseModelConfig, body: any): Promise<any> {
    ...
    try {
      ...
      throw new Error(`Error invoking Bedrock model: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
}
