...
class AnyscaleAPI implements Finetunable, Inferable {
  ...
  public async generate(
    ...
    const unsupportedParams = Object.keys(kwargs).filter(
      key => !LLM_GENERATION_PARAMETERS.includes(key)
    );
    if (unsupportedParams.length > 0) {
      console.warn(
        `Unused generation parameters sent as input: [${unsupportedParams.join(
          ', '
        )}]. For Anyscale, only the following parameters are supported: [${LLM_GENERATION_PARAMETERS.join(
          ', '
        )}]`
      );
    }
    ...
    const params = {
      ...
      messages: [
        ...
        {
          role: 'user',
          content: prompt + (model.parsingHelperTokens.startToken || ''),
        },
      ],
    };
      ...
      try {
        ...
        if (model.parsingHelperTokens.endToken) {
          ...
          if (
            ...
            choice.includes(model.parsingHelperTokens.startToken)
          ) {
        ...
        if (
          response &&
          response.data &&
          response.data.error.code === 'invalid_api_key'
        ) {
          throw new Error(
            `The supplied Anyscale API key ${this.apiKey} is invalid`
          );
        }
        if (counter === 5) {
          throw new Error(
            `Anyscale API failed to generate a response: ${error}`
          );
        }
        counter++;
        await new Promise(resolve =>
          setTimeout(resolve, Math.pow(2, counter) * 1000)
        ); // Exponential backoff
      }
    }
  ...
  public async listFinetuned(
    ...
    try {
      ...
      return response.data.map((job: FineTuningJob) =>
        this.createFinetuneJob(job, modelConfig)
      );
    } catch (error) {
  ...
  public async getFinetuned(
    ...
    this.checkApiKey();
    try {
      const response: FineTuningJob =
        await this.client.fineTuning.jobs.retrieve(jobId);
      return this.createFinetuneJob(response, modelConfig);
    } catch (error) {
