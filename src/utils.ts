...
function approximateTokenCount(content: string): number {
  ...
  const numberOfSpecialCharacters = Array.from(commonSpecialCharacters).reduce(
    (count, char) => {
...
function userDataDir(
  ...
  switch (process.platform) {
    case 'win32': {
      const appData =
        process.env[roaming ? 'APPDATA' : 'LOCALAPPDATA'] ||
        join(homedir(), 'AppData', roaming ? 'Roaming' : 'Local');
      appauthor = appauthor === undefined ? appname : appauthor;
      if (appname) {
        if (appauthor != null) {
          path =
            appauthor !== false
              ? join(appData, appauthor, appname)
...
function encodeInt(n: number): string {
  ...
  const charset = Array.from({ length: 95 }, (_, i) =>
